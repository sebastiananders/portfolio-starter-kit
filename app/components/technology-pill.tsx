...
export function TechnologyPill({ technology, isSelected, onClick }: TechnologyPillProps) {
  const projectCount = technology.projectIds.length
  ...
  return (
    <button
      ...
      aria-label={`${technology.name}${projectCount > 0 ? ` - used in ${projectCount} project${projectCount > 1 ? 's' : ''}` : ''}`}
    >
      {technology.name}
      {projectCount > 0 && (
